...
@registry.register('LSTRHDD')
class LSTRDataLayer(data.Dataset):

    def __init__(self, cfg, phase='train'):
        self.data_root = cfg.DATA.DATA_ROOT
        self.sensor_feature = cfg.INPUT.VISUAL_FEATURE
        self.target_perframe = cfg.INPUT.TARGET_PERFRAME
        self.sessions = getattr(cfg.DATA, phase.upper() + '_SESSION_SET')
        self.long_memory_length = cfg.MODEL.LSTR.LONG_MEMORY_LENGTH
        self.long_memory_sample_rate = cfg.MODEL.LSTR.LONG_MEMORY_SAMPLE_RATE
    ...
    def __getitem__(self, index):
        ...
        sensor_inputs = np.load(
            osp.join(self.data_root, self.sensor_feature, session + '.npy'), mmap_mode='r')
        ...
        if self.future_num_samples > 0:
            ...
            work_indices = np.arange(work_start, work_end).clip(0)
            work_indices = work_indices[::self.work_memory_sample_rate]
            work_sensor_inputs = sensor_inputs[work_indices]

        else:
            # print(work_start,work_end,self.work_memory_sample_rate)
            target = target[work_start: work_end][::self.work_memory_sample_rate]
            work_indices = np.arange(work_start, work_end).clip(0)
            work_indices = work_indices[::self.work_memory_sample_rate]
            work_sensor_inputs = sensor_inputs[work_indices]
        ...
        if self.long_memory_num_samples > 0:
            ...
            if self.training:
                ...
                long_indices = self.uniform_sampler(
                    ...
                    self.long_memory_num_samples,
                    self.long_memory_sample_rate).clip(0)
            long_sensor_inputs = sensor_inputs[long_indices]
            # long_motion_inputs = motion_inputs[long_indices]
        ...
        # Get all memory
        if long_sensor_inputs is not None:
            fusion_sensor_inputs = np.concatenate((long_sensor_inputs, work_sensor_inputs))

        else:
            fusion_sensor_inputs = work_sensor_inputs


        # Convert to tensor
        fusion_sensor_inputs = torch.as_tensor(fusion_sensor_inputs.astype(np.float32))
        target = torch.as_tensor(target.astype(np.float32))
        ...
        if memory_key_padding_mask is not None:
            ...
            if future_target is not None:
                future_target = torch.as_tensor(future_target.astype(np.float32))
                # print(target.shape,future_target.shape)
                return fusion_sensor_inputs,fusion_sensor_inputs, memory_key_padding_mask, (target, future_target)
            # print(target.shape)
            return fusion_sensor_inputs, fusion_sensor_inputs,memory_key_padding_mask, target
        else:
            if future_target is not None:
                future_target = torch.as_tensor(future_target.astype(np.float32))
                return fusion_sensor_inputs , fusion_sensor_inputs,(target, future_target)
            return fusion_sensor_inputs, fusion_sensor_inputs,target
        ...
        if self.training and self.clip_mixup_rate > 0:
            ...
            for segment in segments_to_mix.iterrows():
                ...
                if len(valid_segments) == 0:
                    ...
                    # if new_end_tick - new_start_tick - len(sel_indices) + 1 <= 0:
                    #     shift = np.random.randint(1)
                    #     print(new_end_tick - new_start_tick - len(sel_indices) + 1)
                    #     continue
                    # else:
