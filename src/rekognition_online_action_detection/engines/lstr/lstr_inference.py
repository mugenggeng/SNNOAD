...
from ..base_inferences.perframe_det_batch_inference import do_perframe_det_batch_inference, do_ek100_perframe_det_batch_inference
...
def do_lstr_batch_inference(cfg,
    ...
    if cfg.MODEL.LSTR.INFERENCE_MODE == 'stream':
        ...
        if cfg.DATA.DATA_NAME == 'EK100':
            do_ek100_perframe_det_batch_inference(cfg,
                                            model,
                                            device,
                                            logger)
        else:
...
def do_lstr_stream_inference(cfg, model, device, logger):
    ...
    with torch.no_grad():
        ...
        for session_idx, session in enumerate(cfg.DATA.TEST_SESSION_SET):
            ...
            end_time = time.time()
            fps = (len(gt_targets) / (end_time - start_time))
            fps_total += fps
            num = num + 1
            logger.info('mAP of video {}: {:.5f}'.format(session, result['mean_AP']))

            logger.info('Running time: {:.3f} seconds'.format(end_time - start_time))
            logger.info('frams: {:.3f} seconds'.format(len(gt_targets)))
            logger.info('fps of video {}: {:.1f}'.format(session, fps))
    avg_fps = fps_total/num
    print('fps of all video: {:.1f}'.format(avg_fps))
