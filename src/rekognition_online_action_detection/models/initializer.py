...
def _no_grad_normal_(tensor, mean=0., std=1.):
    with paddle.no_grad():
        tensor.fill_(paddle.normal(mean=mean, std=std, shape=tensor.shape))
    return tensor


def _no_grad_fill_(tensor, value=0.):
    with paddle.no_grad():
        tensor.fill_(paddle.full_like(tensor, value, dtype=tensor.dtype))
    return tensor
...
def vector_(tensor, vector):
    with paddle.no_grad():
        tensor.fill_(paddle.to_tensor(vector, dtype=tensor.dtype))
    return tensor
