...
# import mamba_ssm.selective_scan_fn
...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        x_hwwh = torch.stack([x,x], dim=1).view(B, 2, -1, L)
    ...
    def forward_corev0_seq(self, x: torch.Tensor):
        ...
        y = out_y[:, 0] + inv_y[:, 0] + wh_y + invwh_y
        y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, -1, L)
        y = self.out_norm(y).to(x.dtype)
    ...
    def forward_corev1(self, x: torch.Tensor):
        ...
        inv_y = torch.flip(out_y[:, 2:4], dims=[-1]).view(B, 2, -1, L)
        wh_y = torch.transpose(out_y[:, 1].view(B, -1, L), dim0=2, dim1=3)
        invwh_y = torch.transpose(inv_y[:, 1].view(B, -1, L), dim0=2, dim1=3)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        B, H, L = x.shape
        # print(x.shape,'x.shap111')
        xz = self.in_proj(x)
        x, z = xz.chunk(2, dim=-1) # (b, h, w, d)
        # print(x.shape, 'x.shape22')
        x = x.permute(0, 2, 1)
        # print(x.shape,'x.shape33')
...
def test_vssm():
    ...
    torch.manual_seed(time.time()); torch.cuda.manual_seed(time.time())
    # oldvss = VMamba2Dp(depths=[2,2,6,2]).half().cuda()
    newvss = VSSM(depths=[2,2,9,2]).half().cuda()
    # newvss.load_state_dict(oldvss.state_dict())
    input = torch.randn((4, 3, 224, 224)).half().cuda()
    # torch.cuda.manual_seed(0)
    # with torch.cuda.amp.autocast():
    #     y1 = oldvss.forward_backbone(input)
    torch.cuda.manual_seed(0)
    with torch.cuda.amp.autocast():
        y1 = newvss.forward_features(input)
        y2 = newvss.forward_backbone(input)
        y3 = newvss.forward(input)
    print('forward_features', y1.shape)
    print('forward_backbone', y2.shape)
    print('forward', y3.shape)
    # print((y1 -y2).abs().sum()) # tensor(0., device='cuda:0', grad_fn=<SumBackward0>)
