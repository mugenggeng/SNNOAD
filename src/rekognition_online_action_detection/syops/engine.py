...
def get_syops_pytorch(model, input_res, dataloader=None,
    ...
    if dataloader is not None:
        ...
        for batch, data in enumerate(dataloader, start=1):
            batch_idx += 1
            target = data[-4]
            input = [x for x in data[:-4]]

            torch.cuda.empty_cache()

            batch1 = input[0].float().to(next(syops_model.parameters()).device)  # torch.Size([128, 3, 32, 32])
            batch2 = input[1].float().to(next(syops_model.parameters()).device)  # torch.Size([128, 3, 32, 32])
            # print(batch1.shape,batch2.shape)
            with torch.no_grad():
                # calculate acc at the same time, to confirm the checkpoint
                output = syops_model(batch1, batch2)
        ...
        if input_constructor:
            ...
            try:
                input = []
                for size in input_res:
                    batch = torch.ones(()).new_empty((16, *size),
                                                     dtype=next(syops_model.parameters()).dtype,
                                                     device=next(syops_model.parameters()).device)
                    input.append(batch)
            except StopIteration:
                batch = torch.ones(()).new_empty((16, *input_res))
            
            _ = syops_model(input[0],input[1])
